...
from autogen_agentchat.base import Response, Team, TaskResult
...
from autogen_agentchat.state import AssistantAgentState, SocietyOfMindAgentState
...
class AssistantAgentConfig(BaseModel):
    ...
    show_llm_context: bool = False  # 控制是否显示LLM上下文
...
class AssistantAgent(BaseChatAgent, Component[AssistantAgentConfig]):
    ...
    * The tool calls and results are passed to the target agent through :attr:`~autogen_agentchat.messages.HandoffMessage.context`.
    ...
    Args:
        ...
        show_llm_context (bool, optional): If `True`, the LLM context messages will be included in the response. Defaults to `False`.
    ...
    def __init__(
        ...
        *,
        ...
        self._show_llm_context = show_llm_context
    ...
    async def on_messages_stream(
        ...
        # 根据show_llm_context参数决定是否输出LLM上下文消息
        if self._show_llm_context:
            yield TextMessage(
                source=self.name,
    ...
    def _to_config(self) -> AssistantAgentConfig:
        ...
        return AssistantAgentConfig(
            ...
            show_llm_context=self._show_llm_context,
        )
    ...
    def _from_config(cls, config: AssistantAgentConfig) -> Self:
        ...
        return cls(
            ...
            show_llm_context=config.show_llm_context,
        )

class StreamingSocietyOfMindAgentConfig(BaseModel):
    """The declarative configuration for a StreamingSocietyOfMindAgent."""
    name: str
    team: ComponentModel
    model_client: ComponentModel
    description: str
...
class StreamingSocietyOfMindAgent(BaseChatAgent, Component[StreamingSocietyOfMindAgentConfig]):
    """改进版SocietyOfMindAgent，支持流式输出。"""

    component_config_schema = StreamingSocietyOfMindAgentConfig
    component_provider_override = "autogen_agentchat.agents.StreamingSocietyOfMindAgent"
    ...
    def __init__(
        ...
        model_client: ChatCompletionClient,
        *,
        description: str = "An agent that uses an inner team of agents to generate responses with streaming support.",
        instruction: str = DEFAULT_INSTRUCTION,
        model_client_stream: bool = True,  # 默认启用流式输出
    ) -> None:
        super().__init__(name=name, description=description)
        self._team = team
        self._model_client = model_client
    ...
    def _to_config(self) -> StreamingSocietyOfMindAgentConfig:
        return StreamingSocietyOfMindAgentConfig(
            name=self.name,
    ...
    @classmethod
    def _from_config(cls, config: StreamingSocietyOfMindAgentConfig) -> Self:
        model_client = ChatCompletionClient.load_component(config.model_client)
