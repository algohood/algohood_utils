...
class TargetBase(abc.ABC):
    ...
    def add_sample(self, _sample: Sample):
        """
        add sample to cache
        :param _sample: sample structure:
            class Sample:
    ...
    @abc.abstractmethod
    def update_targets(self, _data: Dict[str, List[List]]) -> Optional[List[Sample]]:
        """
        generate samples that could be used as input in generate_model
        :param _data: {symbol: [[recv_ts, delay, price, amount, direction], ...], ...}
        :return: return list of samples, and sample structure is as follows:
            class Sample:
                signal: Optional[Signal]
                features: Optional[Dict[str, float]]
                targets: Optional[Dict[str, float]]
...
class InterceptBase(abc.ABC):  # 继承 abc.ABC
    @abc.abstractmethod
    def update_model(self, _fnts: List[Dict[str, Dict[str, float]]]) -> None:
        """
    ...
    @abc.abstractmethod
    def forcast_target(self, _features: Dict[str, float]) -> Dict[str, float]:
        """
        forcast target base on the model
        :param _features: features that is generated by generate_features
        :return: forcasted target
        """
        pass

    @abc.abstractmethod
    def check_target(self, _target: Dict[str, float]) -> bool:
        """
        check whether or not to intercept signal base on the model forcast
        :param _target: target that is generated by generate_targets
